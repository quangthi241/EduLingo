...
export function ListeningForm({
  ...
  initial?: Partial<ListeningFormValue>;
  onSubmit: (value: ListeningFormValue) => void;
  submitting?: boolean;
}) {
  const [value, setValue] = useState<ListeningFormValue>({
    meta: initial?.meta ?? {
      slug: "",
      title: "",
      cefr: "B1",
  ...
  return (
    ...
    >
      ...
      <Textarea
        ...
        onChange={(e) => setValue({ ...value, transcript: e.target.value })}
      />
      <p className="text-xs text-ink-muted">
        Audio is uploaded after the draft is saved.
      </p>
      <MCQBuilder
        value={value.mcq}
